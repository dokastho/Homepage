...
def update_topic(tn: int):
    ...
    cur = connection.execute(
        ...
        "WHERE logname == ? "
        "AND topicId == ?",
        (body["title"], body["description"], body["body"], body["route"], logname, tn, )
    )
...
def create_topic():
    ...
    cur = connection.execute(
        ...
        "VALUES (?, ?, ?, ?)",
        (body["title"], body["description"], logname, body["topicId"], )
    )
...
def fetch_topics_by_owner():
    ...
    content = dict({"logname": get_logname(), "topics": cur.fetchall()})
